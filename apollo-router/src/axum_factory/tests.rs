...
async fn it_sends_bad_accept_header() -> Result<(), ApolloRouterError> {
    ...
    assert_eq!(
        response.text().await.unwrap(),
        r#"{"message":"'accept' header can't be different from \\\"*/*\\\", \"application/json\", \"application/graphql-response+json\", \"multipart/mixed;boundary=\\\"graphql\\\"\" or \"multipart/mixed;boundary=\\\"graphql\\\";deferSpec=20220824\"","extensions":{"code":"INVALID_ACCEPT_HEADER"}}"#
    );
