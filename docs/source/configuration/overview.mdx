...
Where indicated, some of these options can also be provided via an environment variable. If an option is provided _both_ ways, the command-line value takes precedence over the environment variable.
...
**Do not set this option in production.** If provided, the Apollo Router runs in dev mode to help with local development.
