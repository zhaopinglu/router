...
That's it! Running the Router with the `--dev` flag enables a development mode that exposes [Apollo Sandbox](/studio/explorer/sandbox/) so you can run queries against the Apollo Router. This flag also relaxes some defaults to provide a better development experience (e.g., it exposes subgraph error messages), so you should always _exclude_ this flag when running the router in a non-development environment.
